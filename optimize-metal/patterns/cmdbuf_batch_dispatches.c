...
//       assumes it.
// SPEEDUP: 1.5–1.8× decode (a13d0de in the gpt-oss reference impl was +78%).
// CAVEAT: Each forward() must end with commit+wait so subsequent host
//         reads (e.g., the argmax result) are visible.
//
// This is the structural pattern, not a drop-in.  See src-metal/main.c
// forward() for a complete example.

#include "metal_shim.h"

static gpu_cmdbuf* g_cmdbuf;   // active cmdbuf during forward()
...
void forward(int q_off, int Lq) {
    ...
    for (int li = 0; li < N_LAYERS; li++) {
        ...
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_rmsnorm,    args_rms,    ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_linear_q,   args_qproj,  ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_linear_k,   args_kproj,  ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_linear_v,   args_vproj,  ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_rope,       args_rope_q, ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_rope,       args_rope_k, ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_sdpa,       args_sdpa,   ...);
        gpu_cmdbuf_dispatch(g_cmdbuf, pso_linear_o,   args_oproj,  ...);
        // ... rest of the layer ...
    }
...
void decode_step(int q_off) {
    g_cmdbuf = gpu_cmdbuf_new(g_ctx);
    forward(q_off, 1);
    gpu_cmdbuf_commit_wait(g_cmdbuf, NULL);
    g_cmdbuf = NULL;
