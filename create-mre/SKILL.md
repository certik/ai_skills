...
description: >
  ...
  run with LFortran but works with other Fortran compilers (e.g., Flang,
  GFortran). Produces standalone .f90 file(s) and a run.sh script that
...
with Flang but fail with LFortran, exhibiting the same error as the original
third-party code.
...
- `lfortran` available on PATH (typically it points to a locally-built version
  at `src/bin/lfortran`)
...
3. Test after each removal:
   - Does `flang` still compile/run it successfully?
...
flang -o test_gfortran <mre_file>.f90 && ./test_gfortran
...
```bash
# Both compile
flang -o test_gfortran <mre_file>.f90
...
Create an executable bash script that reproduces the bug:
...
$GFORTRAN -o test_gfortran <file(s)>
./test_gfortran
echo "flang: OK"
...
# Cleanup
rm -f test_gfortran test_lfortran a.out *.mod
```
...
The MRE compiles with flang but fails with lfortran.
```
