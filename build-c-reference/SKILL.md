...
1. In `main.c`, write `forward(int q_off, int Lq)` (or however your model
   ...
   - For **non-causal / diffusion-style models**, the simplest reference
     ...
     "Discrete-diffusion / masked LMs" and "Fast-dLLM-style block
     diffusion with KV cache" for the cached variant (much faster, also
     the realistic deployment target).
...
#### Fast-dLLM-style block diffusion with KV cache
...
### Multiple samplers / multiple Python references for the same model
...
- **Within-block vs full-sequence logit shift** (Fast-dLLM-style
  samplers): the `cat(logits[:,:1], logits[:,:-1], dim=1)` shift in
  the Python reference is applied to the actual `logits` tensor that
  comes out of the call — full sequence for the prefetch
  (`[L, V]`), one block for refinement (`[BL, V]`). The shift is
  **always relative to that tensor's own row indexing**, NOT relative
  to the absolute sequence position. Implement it as a row-shift over
  the array, not as a "use absolute position p-1" trick.
